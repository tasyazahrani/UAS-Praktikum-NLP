...
def get_processed_files(results):
    """Ambil daftar file yang sudah diproses"""
...
def main():
    ...
    # Filter audio yang belum diproses
    remaining = [f for f in audio_files if os.path.basename(f) not in processed_files]
    print(f"Sudah diproses sebelumnya: {len(processed_files)}")
    ...
    # Ringkasan akhir
    sukses = sum(1 for r in results if not r["error"] and r["llm"] and not r["llm"].startswith("[ERROR]"))
    ...
    print(f"")
    for r in results:
        status = "✅" if not r["error"] and r["llm"] and not r["llm"].startswith("[ERROR]") else "❌"
